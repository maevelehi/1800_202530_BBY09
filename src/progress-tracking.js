...
function build7DayCounts(cards) {
  ...
  cards.forEach(card => {
    ...
    const lastFlipped = card.lastFlipped ? card.lastFlipped.toDate ? card.lastFlipped.toDate() : new Date(card.lastFlipped) : null;
    ...
    const idx = days.findIndex(d => new Date(d).toDateString() === lastFlipped.toDateString());
...
function renderChartAndStats(counts) {
  ...
  document.getElementById("weekly-stats").textContent = `🔥 You’re ${Math.abs(percent)}% ${percent >= 0 ? 'above' : 'below'} your weekly average!`;
}
...
function renderStreakAndMotivation(counts) {
  ...
  document.getElementById("streak-msg").textContent = streak > 0 ? "Keep going, consistency is key!" : "Start today and build your streak!";
  const motivationMsg = streak >= 5 ? "💪 Amazing! You're building a strong habit!" : "💪 Great job today! Keep the streak alive and your brain will thank you!";
